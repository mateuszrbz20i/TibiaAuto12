...
class Runemaker:
    def __init__(self, ManaLocation, MOUSE_OPTION):
        self.AutoMana = GUI('Runemaker', 'Module: Runemaker')
        self.AutoMana.DefaultWindow('Runemaker', [306, 272], [1.2, 2.29])
        ...
        def SetAutoMana():
            ...
            if not EnabledAutoMana:
                ButtonEnabled.configure(text='Runemaker: ON', relief=SUNKEN, bg=rgb((158, 46, 34)))
                print("Runemaker: ON")
                ...
                EnabledAutoMana = False
                CheckingButtons()
                ButtonEnabled.configure(text='Runemaker: OFF', relief=RAISED, bg=rgb((127, 17, 8)))
        ...
        StageOne = self.AutoMana.addCheck(ManaCheckStageOne, [17, 55], InitiatedManaCheckStageOne, "Enable Mana potion\n(below)")
        StageTwo = self.AutoMana.addCheck(ManaCheckStageTwo, [17, 105], InitiatedManaCheckStageTwo, "Enable Rune\n(above)")
